...
p_7000 = polyfit(FAMEN(3,xuhao_7000_fit),UI_FFT_waveletAverage(3,xuhao_7000_fit,2),5);
...
p(k,:) = polyfit(FAMEN(k,xuhao),UI_FFT_waveletAverage(k,xuhao,2),5);
end
...
p_13000 = polyfit(FAMEN(15,xuhao_13000_large),UI_FFT_waveletAverage(15,xuhao_13000_large,2),5);
...
%若已知两条曲线的所有值
Iri_1 = @(RS, alpha) (alpha<70).*(Iri_13000_alpha(alpha)-a(alpha)*(1-q(kq)^((13000-RS)/500))/(1-q(kq)));
...
for k=3:15
    RS1=DataInfo.condition(k,1);
    xuhao=find(FAMEN(k,:)<famen_range(1) & FAMEN(k,:)>famen_range(2));
    I_RI_exp=(UI_FFT_waveletAverage(k,xuhao,2)-Iri_1_real(RS1,70))/max_13000_35*xishu;
...
% ylabel('$\sigma=\sqrt{\sum_{i}(I_{RIexp}(i)-I_{RIbase}(i))^2/N}/I_{RIbase}(N)$','interpreter','latex','FontSize',20)
...
for k=3:15
    RS1=DataInfo.condition(k,1);
    xuhao=find(FAMEN(k,:)<famen_range(1) & FAMEN(k,:)>famen_range(2));
    plot(FAMEN(k,xuhao),(UI_FFT_waveletAverage(k,xuhao,2)-Iri_1_real(RS1,70))/max_13000_35*xishu)
    hold on
    plot(alpha1,Iri_2(RS1,alpha1))
end
set(gca, 'XDir','reverse')
...
famen_range=[95 24];
alpha1=famen_range(1):-1:famen_range(2);
jet_color=colormap(jet(13));

% h2=figure
for k=3:15
    RS1=DataInfo.condition(k,1);
    xuhao=find(FAMEN(k,:)<famen_range(1) & FAMEN(k,:)>famen_range(2));
    AAAAA_WAVELET1(k,xuhao)=(UI_FFT_waveletAverage(k,xuhao,2)-Iri_1_real(RS1,70))/max_13000_35*xishu;
    plot(FAMEN(k,xuhao),AAAAA_WAVELET(k,xuhao),'Color',jet_color(k-3+1,:),'LineWidth',2)
    hold on
    xuhao_point=find(FAMEN(k,:)<31 & FAMEN(k,:)>29);
%     scatter(FAMEN(k,xuhao_point),(UI_FFT_noAverage(k,xuhao_point,2)-Iri_1_real(RS1,70))/max_13000_35*xishu,'o','k')
%     plot(alpha1,Iri_2(RS1,alpha1))
end
set(gca, 'XDir','reverse')
xlim([25 90])
...
xlabel('磁阀开度(%)','FontSize',20);
% 设置其余坐标区属性
% title('不同转速的E_R_I','FontSize',20)
set(gca,'FontSize',20);
% saveas(h1,[save_directory,'/','FIG-所有转速-无平均-方差','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
% line([30,30],[0,1],'linestyle','--','Color','k');

saveas(h2,[save_directory,'/','FIG01-raw2-转速'],'eps');
saveas(h2,[save_directory,'/','FIG01-raw2-转速'],'pdf')
saveas(h2,[save_directory,'/','FIG01-raw2-转速'],'fig')
