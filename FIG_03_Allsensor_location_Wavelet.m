...
amplitude_freq=amplitude_freq3;
UI_FFT=UI_FFT_waveletAverage;
the_freq=the_freq3;
...
%         Level=1/1000;
%         plot(DataInfo.sensorLoc,Level*permute(UI_FFT(i_rotorspeed,i_file,:),[3,1,2]),'+-','LineWidth',0.5,'Color',jet_color(i_file,:))
%         hold on
%     end
%     % famen=[30:1:75];
...
%     %     plot(PI1(xuhao(k),:)/maximum1,DataInfo.sensorLoc,'+-','LineWidth',0.5,'Color',jet_color1(length(famen)-k+1,:));
%     %     hold on
%     % end
%     grid on
%     set(gca,'FontSize',20,'YGrid','on','XTick',DataInfo.sensorLoc,...
%         'XTickLabel',DataInfo.sensorArray);
%     set(gca,'FontSize',20);
%     ylabel('E_{RI}/pa^2x10^3','FontSize',20);
% %     title('不同位置频带能量dui','FontSize',20)
%     % xlim([0 1])
%     xlim([-1 101]);

    %
    subplot('position',[0.1 0.1 0.8 0.8])
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),10),'LineWidth',1,'Color','k')
            hold on

    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),9),'LineWidth',1,'Color',[0.6350 0.0780 0.1840])
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),8),'LineWidth',1,'Color',[0.4940 0.1840 0.5560])
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),7),'LineWidth',1,'Color',[0.3010 0.7450 0.9330])
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),6),'LineWidth',1,'Color',[96 96 96]/255)
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),5),'LineWidth',1,'Color','y')
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),4),'+-','LineWidth',1,'Color',[0.9290 0.6940 0.1250])
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),3),'o-','LineWidth',1,'Color','b')
    plot(UI_FFT_stftAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),2),'^-','LineWidth',1,'Color','r')

    plot(UI_FFT_stftAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),1),'-','LineWidth',1,'Color','g')
    ...
    annotation('textbox',...
        ...
        'Color',[0.84 0.16 0]);
    set(gca,'FontSize',20);
%     plot(1/2400*UI_FFT_waveletAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),2),'+-','LineWidth',0.5,'Color','g')
%     plot(1/2400*UI_FFT_waveletAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),1),'+-','LineWidth',0.5,'Color','r')
    
    
    hold off

    maxUI_FFT=max(UI_FFT_stftAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),2));
    %添加SI虚线
    line([DataInfo.condition(i_rotorspeed,2),DataInfo.condition(i_rotorspeed,2)],[0,maxUI_FFT*1.2],'linestyle','--','Color','k');
    text(DataInfo.condition(i_rotorspeed,2),2.3e5,'SI','FontSize',28);
    ...
    % 创建 ylabel
    % ylabel({'PI（）'});
    ylabel('Wavelet频带能量E_R_I','FontSize',20)
    % 创建 xlabel
    title('不同传感器位置的影响','FontSize',20);
    % 设置其余坐标区属性
%     title('FFT频带能量E_{RI}','FontSize',20)
    set(gca,'FontSize',20);
     box off
...
saveas(h1,[save_directory,'/','FIG03-AllsensorEri-enlarged-wavelet-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'eps')
saveas(h1,[save_directory,'/','FIG03-AllsensorEri-enlarged-wavelet-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'fig')
saveas(h1,[save_directory,'/','FIG03-AllsensorEri-enlarged-wavelet-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
