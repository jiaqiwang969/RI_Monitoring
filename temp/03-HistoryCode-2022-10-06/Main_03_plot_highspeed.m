...
% %为了节省时间，下面代码若提前计算，则只需导入即可。
% %         [the_freq{k,i_file},amplitude_freq{k,i_file}]=fftPlot_dB_universal_feature(Process.sst1{k,i_file},Process.fsResample,2.56,DataInfo.condition(k,1));
% %         %计算频谱能量
...
%% 1. 做出频带的傅里叶频谱、不同位置的UI、不同磁阀开度的UI联合图。
% 每个转速出一张联合图
...
% % 累加频带计算PI：$PI(\sum_{f1}^{f2}(W(fk)))$
% xuhao_init=DataInfo.xuhao(label,1);
% xuhao_end=DataInfo.xuhao(label,end);
% for i_file=xuhao_init:xuhao_end
%     PI1(i_file,:)=sum(Wavlet.global_ws{1,i_file}(RIband,:))/length(RIband);
%     PI2(i_file,:)=sum(Wavlet.global_ws{1,i_file}(band2,:))/length(band2);
% end
...
i_rotorspeed=15; %固定一个转速
% for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
         amplitude_freq_dB=modified*20*log10(sqrt(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel))./2e-5);
...
for  i_file=1:DataInfo.condition(i_rotorspeed,2)
    ...
    the_freq_temp=the_freq{i_rotorspeed,i_file};
    plot(the_freq_temp,modified*smooth(amplitude_freq_dB,60),'.-','LineWidth',1,'MarkerSize',1,'Color',jet_color(i_file,:))
    hold on
end
...
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),10),'LineWidth',2,'Color','k')
hold on
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),9),'LineWidth',2,'Color',[0.6350 0.0780 0.1840])
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),8),'LineWidth',2,'Color',[0.4940 0.1840 0.5560])
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),7),'LineWidth',2,'Color',[0.3010 0.7450 0.9330])
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),6),'LineWidth',2,'Color',[96 96 96]/255)
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),5),'LineWidth',2,'Color',[0.9290 0.6940 0.1250])
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),4),'LineWidth',2,'Color','y')
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),3),'LineWidth',2,'Color','b')
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),2),'LineWidth',2,'Color','g')
plot(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),1),'LineWidth',2,'Color','r')
hold off

maxUI_FFT=max(modified*UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),2));
%添加SI虚线
...
saveas(h1,[save_directory,'/','FIG-转速',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
% cleanfigure
