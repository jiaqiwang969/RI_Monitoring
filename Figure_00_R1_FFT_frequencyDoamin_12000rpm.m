...
the_freq=the_freq2;
    ...
    for k=1:length(RIband)
        line([RIband(k),RIband(k)],[115,145],'linestyle','--','Color','k');
    end
    set(gca,'FontSize',14);
    grid on
    xlim([5 28])
    ylim([115,145])
    ylabel('局部放大','FontSize',14)
...
the_freq=the_freq3;
    ...
    fig=subplot('position',[0.5 0.5 0.3 0.4]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(0.0209*amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
        the_freq_temp=the_freq{i_rotorspeed,i_file};
        plot(the_freq_temp,amplitude_freq_dB,'.-','LineWidth',1,'MarkerSize',1,'Color',jet_color(i_file,:))
        hold on
    end
    ...
    title('不同频带的小波幅值谱','FontSize',16)


    %局部放大-且平滑处理
    subplot('position',[0.5 0.11 0.3 0.3])
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        %23.2998该值根据能量换算得到FFT E_RI==Wavelet E_RI
        amplitude_freq_dB=20*log10(0.0209*amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
        the_freq_temp=the_freq{i_rotorspeed,i_file};
        plot(the_freq_temp,amplitude_freq_dB,'.-','LineWidth',1,'MarkerSize',1,'Color',jet_color(i_file,:))
        hold on
