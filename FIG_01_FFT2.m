...
DataInfo.condition(13,:)=[12000;92;25+1/3*10;95];
...
for k=13
    ...
    for  i_file=DataInfo.condition(k,2)%利用好一点的计算机并行计算
        % %         Process.loadMat=[];
        %         for k1=DataInfo.condition(k,2)
...
%         data_broadband{k,i_file}= data_broadband{k,i_file}./std(data_broadband{k,i_file});
    ...
    %
        for iSensor=1:length(Process.sensorParameter.object(sensorLoc))
            parfor  i_file=1:DataInfo.condition(k,2)%利用好一点的计算机并行计算
                ...
                %        figure;tplot(Sx, 1:length(data_broadband{k,i_file}(:,2)), fs); colorbar; xlabel('STFT','FontSize',14); %xlabel('Time (seconds)','FontSize',14); ylabel('Frequency (hz)', 'FontSize',14);
                %        figure(); plot(1:length(data_broadband{k,i_file}(:,2)),[data_broadband{k,i_file}(:,2),xNew(:,1)]); xlabel('Inverse Synchrosqueezing Signal');
                ...
                % figure(); tplot(Tx, 1:length(data_broadband{k,i_file}(:,2)), fs); colorbar; xlabel('STFT Synchrosqueezing','FontSize',14); %xlabel('Time (seconds)','FontSize',14); ylabel('Frequency (hz)', 'FontSize',14);
                % figure(); plot(1:length(data_broadband{k,i_file}(:,2)),[data_broadband{k,i_file}(:,2),xNew(:,1)]); xlabel('Inverse Synchrosqueezing Signal');
            ...
            for i_file=DataInfo.condition(k,2)
    ...
    for iSensor=1:length(Process.sensorParameter.object(sensorLoc))
        parfor  i_file=DataInfo.condition(k,2)%利用好一点的计算机并行计算
            fk=[1:0.5:58];
            pad = 0;                                   % 2次幂0填充
...
for i_rotorspeed=13 %固定一个转速
    ...
    sensorLabel=1;
    fig=subplot('position',[0.1-0.06 0.75+0.025 0.2 0.18]);
    ...
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        ...
        plot(the_freq_temp,amplitude_freq_dB,'.-','LineWidth',1,'MarkerSize',1,'Color','k')
        hold on
    end
    line([8,8],[114,147],'linestyle','--','Color','k');
    line([24,24],[114,147],'linestyle','--','Color','k');
    text(4,153,[' RI带',sprintf('\n'),'下限(8)'])
    text(19,153,[' RI带',sprintf('\n'),'上限(24)']);box off

    % end
    %
    %     set(gca,'XGrid','on','XTick',RIband1,...
    %         'XTickLabel',RIbandName);
    set(gca,'XGrid','on','XMinorTick','on','XTick',[0 10 20 30 40],...
    ...
    ylim([115,160])
    hold off
    xlabel('B_1位置','FontSize',15)
    ylabel('频谱')
    %xlabel('阶次')
        param = struct('XLabel', 'Order', 'YLabel', 'dB');
    DrawAxisWithArrow(gca, param);



    sensorLabel=2;
    fig=subplot('position',[0.1-0.06 0.5+0.025+0.01 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    sensorLabel=3;
    fig=subplot('position',[0.1-0.06+0.25-0.01*1 0.5+0.025+0.01 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    ylim([115,160])
    hold off
    xlabel('R_2位置','FontSize',15)
        param = struct('XLabel', 'Order', 'YLabel', 'dB');
    DrawAxisWithArrow(gca, param);


    sensorLabel=4;
    fig=subplot('position',[0.1-0.06+0.25*2-0.01*2 0.5+0.025+0.01 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    sensorLabel=5;
    fig=subplot('position',[0.1-0.06+0.25*3-0.01*3 0.5+0.025+0.01 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    ylim([115,160])
    hold off
    xlabel('R_4位置','FontSize',15)
        param = struct('XLabel', 'Order', 'YLabel', 'dB');
    DrawAxisWithArrow(gca, param);


    sensorLabel=6;
    fig=subplot('position',[0.1-0.06 0.25+0.025+0.01*2 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    sensorLabel=7;
    fig=subplot('position',[0.1-0.06+0.25-0.01 0.25+0.025+0.01*2 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    ylim([115,160])
    hold off
    xlabel('R_6位置','FontSize',15)
        param = struct('XLabel', 'Order', 'YLabel', 'dB');
    DrawAxisWithArrow(gca, param);


    sensorLabel=8;
    fig=subplot('position',[0.1-0.06+0.25*2-0.01*2 0.25+0.025+0.01*2 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    sensorLabel=9;
    fig=subplot('position',[0.1-0.06+0.25*3-0.01*3 0.25+0.025+0.01*2 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
    ...
    sensorLabel=10;
    fig=subplot('position',[0.1-0.06 0.0+0.025+0.01*3 0.2 0.18]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
...
saveas(h1,[save_directory,'/','FIG01-FFT-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'eps')
saveas(h1,[save_directory,'/','FIG01-FFT-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'fig')
saveas(h1,[save_directory,'/','FIG01-FFT-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
