...
%         nlabel1=find(the_freq1{k,i_file}<RIbandFFT(end) & the_freq1{k,i_file}>RIbandFFT(1));
...
% 
%             %%
%             nlabel2=find(the_freq2{k,i_file}<RIbandFFT(end) & the_freq2{k,i_file}>RIbandFFT(1));
...
%             nlabel3=find(the_freq3{k,i_file}<RIbandWavelet(end) & the_freq3{k,i_file}>RIbandWavelet(1));
...
for i_rotorspeed=13 %固定一个转速
    ...
    fig=subplot('position',[0.1 0.55 0.8 0.37]);
    ...
    line([8,8],[114,145],'linestyle','--','Color','k');
    line([24,24],[114,145],'linestyle','--','Color','k');
    text(5,146,[' RI带',sprintf('\n'),'下限(8)'])
    text(21,146,[' RI带',sprintf('\n'),'上限(24)']);box off
    box off
    ...
    txt = {['转速',num2str(DataInfo.condition(i_rotorspeed,1))],...
        ...
        ['RI带',num2str(RIbandFFT(1)),'-',num2str(RIbandFFT(end))]};
    annotation('textbox',...
        [0.11 0.74 0.1 0.13],...
        'String',txt,...
        'FontSize',13,...
...
%         'XTickLabel',RIbandName);
    ...
    %     for k=1:length(RIbandFFT)
    %         line([RIbandFFT(k),RIbandFFT(k)],[115,150],'linestyle','--','Color','k');
    %     end
    title('R_1位置的傅里叶幅值谱','FontSize',18)
    param = struct('XLabel', 'Order', 'YLabel', 'dB');
    DrawAxisWithArrow(gca, param);

        set(gca,'XGrid','on','XMinorTick','on','XTick',[10 20 30 40],...
        'XTickLabel',{'10','20','30','40'});
    ...
    sensorLabel=2;
    RIband_wavelet=[8:22];
    fig=subplot('position',[0.1 0.1 0.8 0.37]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(1/sqrt(2300)*amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
        the_freq_temp=the_freq{i_rotorspeed,i_file};
        plot(the_freq_temp,amplitude_freq_dB,'.-','LineWidth',1,'MarkerSize',1,'Color',jet_color(i_file,:))
        hold on
    end
    line([8,8],[114,145],'linestyle','--','Color','k');
    line([20,20],[114,145],'linestyle','--','Color','k');
    text(5,146,[' RI带',sprintf('\n'),'下限(8)'])
    text(18,146,[' RI带',sprintf('\n'),'上限(20)']);box off
    box off


    % end
%     set(gca,'XGrid','on','XTick',RIband_wavelet,...
%         'XTickLabel',RIbandName);
   ...
   set(gca,'XGrid','on','XMinorTick','on','XTick',[10 20 30 40],...
    ...
    txt = {['转速',num2str(DataInfo.condition(i_rotorspeed,1))],...
        ['采样',num2str(N_caiyang*29)],['位置',DataInfo.sensorArray{sensorLabel}],...
        ['RI带',num2str(RIbandWavelet(1)),'-',num2str(RIbandWavelet(end))]};
    annotation('textbox',...
        [0.11+0.45 0.74 0.1 0.13],...
        'String',txt,...
        'FontSize',13,...
        'FontName','Arial',...
        'LineStyle','--',...
...
%         'XTickLabel',RIbandName);
        ...
        'XTickLabel',{'10','20','30','40'});
    hold off
    colorbar(gca,'TickLabels',{'100%','92%','84%','76%','68%','60%','52%','44%','36%','28%','开度'});
    %     for k=1:length(RIbandWavelet)
    %         line([RIbandWavelet(k),RIbandWavelet(k)],[115,150],'linestyle','--','Color','k');
    %     end
    title('R_1位置的小波幅值谱','FontSize',18)
    param = struct('XLabel', 'Order', 'YLabel', 'dB');
    DrawAxisWithArrow(gca, param);
...
saveas(h1,[save_directory,'/','FIG02-FFTvsWAVELET-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint)],'eps')
saveas(h1,[save_directory,'/','FIG02-FFTvsWAVELET-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint)],'fig')
saveas(h1,[save_directory,'/','FIG02-FFTvsWAVELET-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint)],'pdf')
