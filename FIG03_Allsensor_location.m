...
the_freq=the_freq2;
    ...
    %不同位置的UI(频带内的能量)
    subplot('position',[0.1 0.11 0.22 0.8])
    ...
    set(gca,'FontSize',20);
    xlabel('E_{RI}/pa^2x10^3','FontSize',20);
%     title('不同位置频带能量dui','FontSize',20)
    % xlim([0 1])
    ylim([-1 101]);

    %
    subplot('position',[0.42 0.11 0.55 0.8])
    ...
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),5),'LineWidth',2,'Color','y')
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),4),'o-','LineWidth',2,'Color',[0.9290 0.6940 0.1250])
    plot(UI_FFT(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),3),'^-','LineWidth',2,'Color','b')
    plot(UI_FFT_stftAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),2),'+-','LineWidth',4,'Color','g')

    plot(UI_FFT_stftAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),1),'-','LineWidth',2,'Color','r')
        txt = {['转速',num2str(DataInfo.condition(i_rotorspeed,1))],...
        ['采样',num2str(N_caiyang*29)],...
        ['RI带',num2str(RIband(1)),'-',num2str(RIband(end))]};
    annotation('textbox',...
        [0.41+0.1 0.5 0.2 0.23],...
        'String',txt,...
        'FontSize',20,...
        'FontName','Arial',...
        'LineStyle','--',...
...
%     plot(1/2400*UI_FFT_waveletAverage(i_rotorspeed,1:DataInfo.condition(i_rotorspeed,2),1),'+-','LineWidth',2,'Color','r')
    ...
    set(legend1,'NumColumns',1,'Location','northwest','FontSize',28);
...
saveas(h1,[save_directory,'/','FIG03-AllsensorEri-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'eps')
saveas(h1,[save_directory,'/','FIG03-AllsensorEri-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'fig')
saveas(h1,[save_directory,'/','FIG03-AllsensorEri-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
