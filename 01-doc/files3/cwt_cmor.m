function coefs = cwt_cmor(sig,Fb,Fc,f,fs)
%利用复morlet小波进行连续小波变换
...
%小波的积分函数
    ...
    dt = t(2) - t(1);
    val_integration = cumsum(cmorwavf(-tlow2low,tlow2low,2^iter,Fb,Fc))*dt;
    %卷积前准备
    t = t-t(1);
    tMax = t(end);   
...
%小波与信号的卷积
    for k = 1:length(scal)    %一个scal一行
        ...
        waveinscal = fliplr(val_integration(j));     %把积分值扩展到j区间，然后左右颠倒。f为当下尺度的积分小波函数
        coefs(k,:) = -sqrt(a)*wkeep1(diff(conv(sig,waveinscal)),L);      %wkeep1取diff(wconv1(ySIG,f))里长度为lenSIG的中间一段
        %
    end
 end