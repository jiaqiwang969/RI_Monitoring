...
%     colorbar(gca,'TickLabels',{'88%','76%','64%','52%','40%','28%'});
    ...
    title('B1位置的小波幅值谱','FontSize',16)

sensorLabel=2;


    fig=subplot('position',[0.55 0.1 0.4 0.8]);
    jet_color=colormap(jet(100));
    % for i_rotorspeed=3 %:size(DataInfo.condition,1) 暂时固定一个转速
    for  i_file=1:DataInfo.condition(i_rotorspeed,2)
        amplitude_freq_dB=20*log10(1/sqrt(2300)*amplitude_freq{i_rotorspeed,i_file}(:,sensorLabel)./2e-5);
...
saveas(h1,[save_directory,'/','FIG01-WAVELET-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'eps')
saveas(h1,[save_directory,'/','FIG01-WAVELET-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'fig')
saveas(h1,[save_directory,'/','FIG01-WAVELET-',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
