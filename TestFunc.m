...
A=[];
for k=3:15
    A(k,:)=UI_FFT_Average(k,DataInfo.condition(k,2)-89:DataInfo.condition(k,2)-89+78,2);
...
% 创建 ylabel
% ylabel({'PI（）'});
ylabel('$I_{RI}=\sum_{i}(P(i)))[pa^2]$','interpreter','latex','FontSize',20)
% 创建 xlabel
xlabel('磁阀开度(%)','FontSize',30);
% 设置其余坐标区属性
title('不同转速的UI','FontSize',14)
set(gca,'FontSize',14);
saveas(h1,[save_directory,'/','FIG-所有转速-平均','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
...
for i_rotorspeed=3:15 %固定一个转速
    ...
    txt = {['转速',num2str(DataInfo.condition(i_rotorspeed,1))],...
        ['采样',N_caiyang*29],['位置',DataInfo.sensorArray{sensorLabel}]};
    annotation('textbox',...
        [0.11 0.77 0.1 0.1],...
        'String',txt,...
        'FontSize',13,...
    ...
    set(gca,'FontSize',14);
    saveas(h1,[save_directory,'/','FIG-平均',num2str(DataInfo.condition(i_rotorspeed,1)),'rpm','-采样率',num2str(resamplePoint),'-RIband-',num2str(RIband(1)),'-',num2str(RIband(end))],'pdf')
end
